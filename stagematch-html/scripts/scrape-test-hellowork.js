...
async function testHelloWork() {
    ...
    try {
        // Test HelloWork URL
        const url = 'https://candidat.francetravail.fr/emplois?motsCles=stage&lieux=Paris&trierPar=date';
        ...
        fs.writeFileSync('c:/tmp/hellowork_snapshot.html', content);
        console.log('HTML snapshot saved to c:/tmp/hellowork_snapshot.html');
        ...
        const jobs = await page.evaluate(() => {
            const jobs = [];
            const cards = document.querySelectorAll('article, .result, .job-card, li');
