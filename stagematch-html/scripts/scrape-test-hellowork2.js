...
/**
 * Test HelloWork page structure with new URL
 */
...
async function testHelloWork() {
    ...
    try {
        // Test with the new France Travail URL
        const url = 'https://www.francetravail.fr/emploi/rechercher';
        ...
        fs.writeFileSync('c:/tmp/hellowork_snapshot2.html', content);
        console.log('HTML snapshot saved to c:/tmp/hellowork_snapshot2.html');

        // Get all job-like elements
        const jobs = await page.evaluate(() => {
            const jobs = [];
            const cards = document.querySelectorAll('article, .result, .job-card, li, .card');
